...
char salida_B = 0;
int salida_UART_temp = 0;
int salida_UART_ruido = 0;
int salida_UART_humedad = 0;
char contador = 0;
char continuar = 0;
...
int leerTemperatura()
{
    int temperatura, salida_ADC; 
    
    ADCON0bits.CHS = 0b1000;         //Elegir el channel 8  del ADC
    //ESPERAR el Acquisition Time  Page 389
    ADCON0bits.GO = 1;              //Iniciar conversion
    while(ADCON0bits.GO == 1);
    
    salida_ADC = (int)ADRESH << 8 | (int)ADRESL;
    temperatura = (((float)(salida_ADC/1024) * 5) / 0.01);                    //Temperatura : VOUT = 10 mv/°C × T   --> T = Vout/0.01
...
int leerHumedad()
{
    int humedad, salida_ADC; 
    
    ADCON0bits.CHS = 0b1010;         //Elegir el channel 10  del ADC
    //ESPERAR el Acquisition Time  Page 389
    ADCON0bits.GO = 1;              //Iniciar conversion
    while(ADCON0bits.GO == 1);
    ...
    humedad = ((((salida_ADC/1024) * 5) - 0.826) / 0.0315);                    //(Vout - 0.826)/0.0315)
...
void __interrupt() treat_int(void)
{
    ...
    if(INTCONbits.T0IF)
    {
        ...
        if(contador >= 200)
        {
            salida_UART_temp = leerTemperatura();
            salida_UART_humedad = leerHumedad();
            salida_UART_ruido = leerRuido();
            continuar = 1;
            contador = 0;
        }
...
void init_Ports(void)
{
    ...
    ANSELHbits.ANS10 = 1;             //Configurar el pin 1 del puerto B como Input Analogico

    //Temperatura
    TRISBbits.TRISB2 = 1;           //Configurar el pin 2 del puerto B como Input
    ANSELHbits.ANS8 = 1;             //Configurar el pin 2 del puerto B como Input Analogico
}
